...
from audio_engine.engine.onset.band_onset_merge import merge_close_onsets
from audio_engine.engine.onset.constants import (
    KEY_ONSET_MIN_SEP_SEC,
    ENERGY_PERCENTILE_THRESHOLD,
)
from audio_engine.engine.onset.drum_band_energy import compute_band_onset_energies
...
def select_key_onsets_by_band(
    ...
    *,
    ...
    for band in ("low", "mid", "high"):
        ...
        # 1차 게이트: percentile 이상만 유지
        th = np.percentile(scores, energy_percentile_threshold)
        mask = scores >= th
        if np.sum(mask) == 0:
